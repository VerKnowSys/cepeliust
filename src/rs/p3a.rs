...
fn main() {
    ...
    let a_string = Some("Mój string".to_string());
    ...
    let b_string = Some("Drugi string".to_string());
    ...
    let if_not_a_then_b_via_fun = b_string.unwrap_or_else(
        moja_fun
    );
    ...
    let no_string = none_of_string.unwrap_or(Some("Nic nie ma, nic nie będzie".to_string()));
